...
<main>
  ...
  <?php if(is_null($login_user['birthday'])): ?>
    <!-- 生年月日に関する記述 -->
    <div class="birthday">
      <div>
  ...
  <?php else: ?>
    <input type="hidden" name="birthday" value="<?php echo Dbc::h($login_user['birthday']); ?>">
  <?php endif; ?>

  <!-- メールアドレスに関する記述 -->
  <div class="email">
    <p class="title">メールアドレス <span>※必須</span></p>
    <input type="email" name="email" id="email" autocomplete="on" value="<?php echo isset($email) ? $email : Dbc::h($login_user['email']); ?>">
  ...
  <!-- 電話番号に関する記述 -->
  <div class="tel">
    <p class="title">電話番号 (ハイフン不要)</p>
    <input type="tel" name="tel" id="tel" autocomplete="on" value="<?php echo isset($tel) ? $tel : Dbc::h($login_user['tel']); ?>">
